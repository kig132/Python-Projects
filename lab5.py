...
"""
APS106 – Lab 5: 簡単なカードゲームの実装

目的:
- リストとネストリストからの情報抽出
- リストの要素追加・削除
- 関数にリストを渡した際のエイリアスの理解
- ループ・条件分岐・ブール式の練習
...
関数:
1. deal_card(hand, deck)
   - デッキの先頭カードを手札に追加
   - 戻り値なし（エイリアスにより変更が反映される）

2. score_cards(hand)
   - 数値カード(2~10): 数値そのまま
   - 絵札(11~13): 10点
   - エース(1): 1点または11点（21を超えない最大値）

3. play(deck)
   - プレイヤーとディーラーに交互に2枚ずつ配る
   - プレイヤー得点 <14 の間、カードを引く
   - プレイヤーが21超え → 敗北
   - ディーラーがプレイヤー未満ならカード引く
   - ディーラーが21超え → 勝利
   - 最終スコアを比較し勝者を判定
   - 戻り値: [勝者の文字列, ディーラーの得点, プレイヤーの得点]

補助関数（編集不要）:
- generate_deck(): 52枚の標準デッキを生成
- shuffle(deck): デッキをシャッフル

テスト方法:
- 方法①：デッキを手動で作る（推奨）
- 方法②：random.seed() を使ってシャッフルの再現性を持たせる
