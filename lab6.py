...
"""
APS106 – Lab 6: 化学反応式のバランスチェックプログラム
...
関数:
1. molecule_formula(molecule_str)
   - 化学式の文字列を辞書に変換（元素: 原子数）
   - 例: "C2H6O1" → {'C':2, 'H':6, 'O':1}

2. expression_formula(expr_coeffs, expr_molecs)
   - 各係数・分子の辞書を使って、式全体の原子数を合算
   - 例: (2,1,5), ({'Na':1,'Cl':1},{'H':2},{'Na':1,'F':1}) 
     → {'Na':7, 'Cl':2, 'H':2, 'F':5}

3. identify_unbalanced_atoms(lhs_dict, rhs_dict)
   - 左右の辞書からバランスが取れていない元素を集合で返す
   - 例: {'C':3,'H':8,'O':10}, {'C':2,'H':8,'O':8} → {'C', 'O'}

4. check_eqn_balance(lhs_expr, rhs_expr)
   - 式全体の原子バランスをチェック
   - 入力: ((係数タプル), (化学式タプル)) × 2（左辺・右辺）
   - 例: ((1,5),("C3H8","O2")), ((4,3),("H2O1","C1O2")) → set()
